...
const SubCategory = ({navigation}) => {
  ...
  const [tableHead] = useState(['ID', 'Category Name', 'Sub Category Name']);
  const [widthArr] = useState([150, 200, 200]);
  ...
  const renderTableRows = () => {
    ...
    return dataToDisplay.map((rowData, index) => (
      <Row
        ...
        data={[
          rowData.id.toString(),
          getCategoryNameById(rowData.catg_id),
          rowData.name,
        ]}
        widthArr={widthArr}
